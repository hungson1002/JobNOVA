...
async function getUser(username: string): Promise<any> {
  try {
    const userRes = await fetch(`http://localhost:8800/api/users/by-username/${username}`);
...
async function getPortfolios(userId: string): Promise<any[]> {
  try {
...
export default function UserProfilePage() {
  ...
  useEffect(() => {
    if (profileUser?.id) {
      getPortfolios(profileUser.id).then(setPortfolios);
    }
  }, [profileUser]);
